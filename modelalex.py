...
def encode_rad(input, n_labels, do=True):
    ...
    l = tf_helpers.get_radian_conv("conv1",input,width = 11,height = 11, dim = 96, stride = 4)
    l = tf_helpers.get_pool_and_lrn(l, 1, ksize=3, strides=2):

    l = tf_helpers.get_radian_conv("conv2",input,width = 5,height = 5, dim = 256, stride = 1)
    l = tf_helpers.get_pool_and_lrn(l, 2, ksize=3, strides=2):

    l = tf_helpers.get_radian_conv("conv3",input,width = 3,height = 3, dim = 384, stride = 1)
    l = tf_helpers.get_radian_conv("conv4",input,width = 3,height = 3, dim = 384, stride = 1)
    l = tf_helpers.get_radian_conv("conv5",input,width = 3,height = 3, dim = 256, stride = 1)
