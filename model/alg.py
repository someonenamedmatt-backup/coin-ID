...
if __name__ == "__main__":
    cs = CoinSampler('data/ebay_coin_data_current.csv','grade_lbl','/data2/',BATCH_LEN, seed = SEED)
    test_sampler = cs.get_test_split()
    # train_model(cs,  test_sampler, '5c2r1sm-v2', iters = len(cs.IDlabel)*25)
    test_model(test_sampler, '5c2r1sm-v2')
    ...
    #     for cl, fn, img in ci:
    #         batch.append(img)
    #     input_tensor = tf.placeholder(tf.float32, [len(batch),90,90,3])
    #
    #     cost = ca.cost(input_tensor)
    #     train = tf.train.AdamOptimizer(0.001).minimize(cost)
    #
    ...
    #             _, loss_value = sess.run([train, cost], {input_tensor: batch})
    #             print("Loss %f" % loss_value)
    #             if b % 10 == 0:
    #                 saver.save(sess, "model.ckpt")
    # elif sys.argv[1] == "-p":
    #     input_tensor = tf.placeholder(tf.float32, [1,90,90,3])
    #     out_tensor = tf.placeholder(tf.float32, [1,1,1,10])
    #     fname = sys.argv[2]
    #     img = ci.get_image(fname)
    #     coded = ca.encode(input_tensor)
    #     decoded = ca.decode(out_tensor)
    #     #pred = ca.decode(ca.encode(input_tensor))
    #     saver = tf.train.Saver()
    #
    #     with tf.Session() as sess:
    #         saver.restore(sess, "model2.ckpt")
    #         code = sess.run(coded, {input_tensor: img.reshape(1,90,90,3)})
    #         img_out = sess.run(decoded, {out_tensor: code})
    #     print(code)
    #     fig = plt.figure()
    #     plt.imshow(img)
    #     fig = plt.figure()
    ...
    #     plt.show()
    # elif sys.argv[1] == "-p2":
    #     get_file_vec(sys.argv[2])
    # elif sys.argv[1] == "-c":
    #     x = get_file_vec(sys.argv[2])
    #     y = get_file_vec(sys.argv[3])
    #     print(np.sum(x*y)/(np.linalg.norm(x)*np.linalg.norm(y)))
    # elif sys.argv[1] == "-r":
    #     files = os.listdir(sys.argv[2])
    #     of = open(sys.argv[3],"w")
    #     for f in files:
    #         of.write(f + "," + ",".join(map(str,get_file_vec(sys.argv[2] + f).reshape(-1) )) + "\n")
