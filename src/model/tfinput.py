...
def _get_grade_final(name):
    grade = int(name.split('-')[-1][:2])
    if grade <= 20:
        return 0
    elif grade <= 50:
        return 1
...
def input(file_list, batch_size, distort_img = False, make_coins = False, name_lbls = None, coin_prop = 'img'):
    ...
    if make_coins:
      for i, f  in enumerate(file_list):
        ...
        try:
            grade_lbl = _get_grade_final(f)
        except:
            grade_lbl = -1
        if name_lbls is not None:
            coin.binarize_coin(f+'_tmp', name_lbl = name_lbls[i], coin_prop = coin_prop, grade_lbl = grade_lbl)
        else:
            coin.binarize_coin(f+'_tmp', coin_prop = coin_prop, grade_lbl = grade_lbl)
